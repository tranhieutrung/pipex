...
/*   Updated: 2024/07/31 21:27:22 by hitran           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
// static void	execute_child_process(t_pipex *pipex, int *pipex->pipe_id, int cmd_index)
// {
// 	int in_fd;
...
// 		read_here_doc(pipex, pipex->pipe_id);
// 		dup2(pipex->pipe_id[1], 1);
...
// 	}
// 	else
// 		redirect_fds(pipex->pipe_id[0], 0, pipex->pipe_id[1], 1);
// 	excecute_command(pipex, pipex->argv[cmd_index]);
// }

// static void	execute_parent_process(t_pipex *pipex, int *pipex->pipe_id)
// {
// 	pid_t	pid;
// 	int		out_fd;
...
// 		handle_fork_error(pipex->pipe_id[0], pipex->pipe_id[1]);
// 	else if (pid == 0)
// 		execute_child_process(pipex, pipex->pipe_id, cmd_index);
// 	else
// 	{
// 		close(pipex->pipe_id[1]);
...
// 		else
// 			execute_parent_process(pipex, pipex->pipe_id);
// 		close(pipex->pipe_id[0]);
// 		waitpid(pid, NULL, 0);
// 	}
// }
...
static void	execute_child_process(t_pipex *pipex, int *pipe_id)
{
	...
	if (ft_strcmp(pipex->argv[1], "here_doc") == 0)
	{
		...
		close (hd_pipe[1]);
		redirect_fds(hd_pipe[0], 0, pipe_id[1], 1);
